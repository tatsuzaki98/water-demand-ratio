program calc_demand
  ...
  !======================================================================
  open (TMP_UID, file='/lake5/tasaka/data_prefecture.csv', action='read')
  do i = 1, 47
    read (TMP_UID, *) j, cpref(i)
  end do
  close (TMP_UID)

  open (TMP_UID, file=trim(land_path)//'Prefecture_'//trim(suffix)//'.bin',&
  &form='unformatted', access='direct', recl=mx*my*ibin, action='read', status='old')
  read (TMP_UID, rec=1) ((pref(i, j), i=1, mx), j=1, my)
  close (TMP_UID)

  open (TMP_UID, file=trim(land_path)//'ClassFrac_'//trim(suffix)//'_ALOS.bin',&
  &form='unformatted', access='direct', recl=mx*my*ibin, action='read', status='old')
  read (TMP_UID, rec=2) ((ocean(i, j), i=1, mx), j=1, my)
  read (TMP_UID, rec=15) ((paddy(i, j), i=1, mx), j=1, my)
  close (TMP_UID)

  open (TMP_UID, file=trim(land_path)//'cropstage_'//trim(suffix)//'_MAFF.bin',&
  &form='unformatted', access='direct', recl=mx*my*ibin, action='read', status='old')
  do t = 1, 36
    read (TMP_UID, rec=t) ((crpstg(i, j, t), i=1, mx), j=1, my)
  end do
  close (TMP_UID)

  open (TMP_UID, file=trim(land_path)//'Area_'//trim(suffix)//'.bin',&
  &form='unformatted', access='direct', recl=mx*my*ibin, action='read', status='old')
  read (TMP_UID, rec=1) ((area(i, j), i=1, mx), j=1, my)
  close (TMP_UID)
